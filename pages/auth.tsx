          ...
          <div
            className={
              cn(
                ...
                theme === 'light' || theme === 'custom' ? 'text-gray-900' : 'text-white'
              )
            }
          >
                ...
                {themes.map(({ label, value }) => (
                  <Button
                    ...
                    className={
                      cn(
                        "w-1/3",
                        theme === value && "bg-gradient-to-r from-pink-500 to-indigo-500 text-white"
                      )
                    }
    ...
    <Card className={themeCardClass[theme]}>
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          <label
            htmlFor="message"
            className={cn('text-md font-medium block mb-1', themeTextClass[theme])}
          >
            Message
          </label>
          <Textarea
            ...
            className={cn(
              theme === 'dark' ? 'bg-gray-700 border-gray-600 placeholder:text-white' : '',
              theme === 'light' ? 'bg-gray-100 border-gray-300 placeholder:text-gray-400' : '',
              theme === 'custom' ? 'bg-[#fffbe6] border-[#fcb69f] placeholder:text-orange-400' : ''
            )}
          />
        </div>
        ...
        {signature && (
          <div className="space-y-2 pt-4">
            <Separator className={theme === 'dark' ? 'bg-gray-700' : theme === 'light' ? 'bg-gray-200' : 'bg-[#fcb69f]'} />
            <label className={cn('text-md font-medium block mb-1', themeTextClass[theme])}>
              Signature
            </label>
            <div className={cn(
              ...
              <p className={cn('text-sm font-mono break-all', themeTextClass[theme])}>
                {signature}
              </p>
            </div>
          </div>
